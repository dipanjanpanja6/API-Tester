const { Employee } = require("../models/Employee")

const create = async (req, res) => {
  try {
    const {
      employeeId,
      employeeFirstName,
      employeeLastName,
      employeeEmail,
      employeeJoiningDate,
      employeePayrollDate,
      employeeHouseNumber,
      employeeStreetNumber,
      employeeCity,
      employeeState,
      employeeZipCode,
      employeeSalary,
    } = req.body

    if (!employeeFirstName) return res.status(400).send({ message: "First name is required" })
    if (!employeeLastName) return res.status(400).send({ message: "Last name is required" })
    if (!employeeEmail) return res.status(400).send({ message: "Email address is required" })
    if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(employeeEmail)) return res.status(400).send({ message: "enter a valid email" })
    if (!employeeJoiningDate) return res.status(400).send({ message: "First name is required" })
    if (!employeePayrollDate) return res.status(400).send({ message: "Employee payroll date is required" })
    if (!employeeHouseNumber) return res.status(400).send({ message: "Employee house number is required" })
    if (!employeeStreetNumber) return res.status(400).send({ message: "Employee street number is required" })
    if (!employeeCity) return res.status(400).send({ message: "Employee city is required" })
    if (!employeeState) return res.status(400).send({ message: "Employee state name is required" })
    if (!employeeZipCode) return res.status(400).send({ message: "Employee Zip Code is required" })
    if (!employeeSalary) return res.status(400).send({ message: "Employee salary is required" })

    await Employee().insert({
      employeeId,
      employeeFirstName,
      employeeLastName,
      employeeEmail,
      employeeJoiningDate,
      employeePayrollDate,
      employeeHouseNumber,
      employeeStreetNumber,
      employeeCity,
      employeeState,
      employeeZipCode,
      employeeSalary,
    })

    return res.status(201).send({ message: "New employee added successfully" })
  } catch (error) {
    return res.status(500).send({ message: "failed", error: error.message })
  }
}

const read = async (_req, res) => {
  try {
    const employees = await Employee()
    return res.status(200).send({ message: "Employee List", data: employees })
  } catch (error) {
    return res.status(500).send({ message: "failed", error: error.message })
  }
}

const update = async (req, res) => {
  try {
    const employeeId = req.params.employeeId
    const isExist = await Employee().where({ employeeId }).first()
    if (!isExist) return res.status(400).send({ message: "Employee Id not exist" })

    const {
      employeeFirstName,
      employeeLastName,
      employeeEmail,
      employeeJoiningDate,
      employeePayrollDate,
      employeeHouseNumber,
      employeeStreetNumber,
      employeeCity,
      employeeState,
      employeeZipCode,
      employeeSalary,
    } = req.body

    if (!employeeFirstName) return res.status(400).send({ message: "First name is required" })
    if (!employeeLastName) return res.status(400).send({ message: "Last name is required" })
    if (!employeeEmail) return res.status(400).send({ message: "Email address is required" })
    if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(employeeEmail)) return res.status(400).send({ message: "enter a valid email" })
    if (!employeeJoiningDate) return res.status(400).send({ message: "First name is required" })
    if (!employeePayrollDate) return res.status(400).send({ message: "Employee payroll date is required" })
    if (!employeeHouseNumber) return res.status(400).send({ message: "Employee house number is required" })
    if (!employeeStreetNumber) return res.status(400).send({ message: "Employee street number is required" })
    if (!employeeCity) return res.status(400).send({ message: "Employee city is required" })
    if (!employeeState) return res.status(400).send({ message: "Employee state name is required" })
    if (!employeeZipCode) return res.status(400).send({ message: "Employee Zip Code is required" })
    if (!employeeSalary) return res.status(400).send({ message: "Employee salary is required" })

    const employee = await Employee().update(
      {
        employeeFirstName,
        employeeLastName,
        employeeEmail,
        employeeJoiningDate,
        employeePayrollDate,
        employeeHouseNumber,
        employeeStreetNumber,
        employeeCity,
        employeeState,
        employeeZipCode,
        employeeSalary,
        employeeId,
      },
      "*"
    )

    return res.status(200).send({ message: "Employee data update successfully", data: employee[0] })
  } catch (error) {
    return res.status(500).send({ message: "failed", error: err.message })
  }
}

const updatePatch = async (req, res) => {
  try {
    const employeeId = req.params.employeeId
    const isExist = await Employee().where({ employeeId }).first()
    if (!isExist) return res.status(400).send({ message: "Employee Id not exist" })

    let {
      employeeFirstName,
      employeeLastName,
      employeeEmail,
      employeeJoiningDate,
      employeePayrollDate,
      employeeHouseNumber,
      employeeStreetNumber,
      employeeCity,
      employeeState,
      employeeZipCode,
      employeeSalary,
    } = req.body

    var query =
      "UPDATE employee set employeeFirstName=IFNULL(?,employeeFirstName), employeeLastName=IFNULL(?,employeeLastName), employeeEmail=IFNULL(?,employeeEmail), employeeJoiningDate=IFNULL(?,employeeJoiningDate), employeePayrollDate=IFNULL(?,employeePayrollDate), employeeHouseNumber=IFNULL(?,employeeHouseNumber), employeeStreetNumber=IFNULL(?,employeeStreetNumber), employeeCity=IFNULL(?,employeeCity), employeeState=IFNULL(?,employeeState), employeeZipCode=IFNULL(?,employeeZipCode), employeeSalary=IFNULL(?,employeeSalary) where employeeId=?"

    if (employeeEmail) {
      if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(employeeEmail)) {
        return res.status(400).send({ message: "enter a valid email" })
      }
    }

    connection.query(
      query,
      [
        employeeFirstName,
        employeeLastName,
        employeeEmail,
        employeeJoiningDate,
        employeePayrollDate,
        employeeHouseNumber,
        employeeStreetNumber,
        employeeCity,
        employeeState,
        employeeZipCode,
        employeeSalary,
        employeeId,
      ],
      (err, results) => {
        if (!err) {
          if (results.affectedRows == 0) return res.status(404).send({ message: "EmployeeId does not found" })
          return res.status(200).send({ message: "Employee data update successfully" })
        } else {
          return res.status(500).send({ message: "failed", error: err.message })
        }
      }
    )
  } catch (error) {
    return res.status(500).send({ message: "failed", error: err.message })
  }
}

const deleteData = (req, res) => {
  try {
    let employeeId = req.params.employeeId

    if (!employeeId) {
      return res.status(400).send("EmployeeId is required")
    }

    var query = "delete from employee where employeeId= ?"

    connection.query(query, [employeeId], (err, results) => {
      if (!err) {
        if (results.affectedRows == 0) {
          return res.status(404).send({ message: "Employee Id does not found" })
        }
        return res.status(200).send({ message: "employee data deleted successfully" })
      } else {
        return res.status(500).send({ message: "failed", error: err.message })
      }
    })
  } catch (error) {
    return res.status(500).send({ message: "failed", error: err.message })
  }
}

module.exports = { create, read, update, updatePatch, deleteData }
